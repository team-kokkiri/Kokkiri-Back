...
public interface DailyRankingRepository extends JpaRepository<DailyRanking, Long> {
    ...
    // 특정 문제의 랭킹 조회 (순위순)
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.id = :problemId ORDER BY dr.rankPosition ASC")
    List<DailyRanking> findByProblemOrderByRank(@Param("problemId") Long problemId);
    
    // 특정 문제의 랭킹 조회 (해결 시간순)
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.id = :problemId ORDER BY dr.solveTime ASC")
    List<DailyRanking> findByProblemOrderBySolveTime(@Param("problemId") Long problemId);
    
    // 특정 날짜의 랭킹 조회
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.problemDate = :problemDate ORDER BY dr.rankPosition ASC")
    List<DailyRanking> findByProblemDateOrderByRank(@Param("problemDate") LocalDate problemDate);
    
    // 오늘 문제의 랭킹 조회
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.problemDate = CURRENT_DATE ORDER BY dr.rankPosition ASC")
    List<DailyRanking> findTodayRankingOrderByRank();
    
    // 특정 회원의 특정 문제 랭킹 조회
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.id = :problemId AND dr.member.id = :memberId")
    ...
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.member.id = :memberId ORDER BY dr.dailyProblem.problemDate DESC")
    ...
    // 특정 문제의 상위 N명 랭킹 조회
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.id = :problemId ORDER BY dr.rankPosition ASC LIMIT :limit")
    List<DailyRanking> findTopRankingsByProblem(@Param("problemId") Long problemId, @Param("limit") int limit);
    
    // 최근 N일간의 랭킹 기록 조회
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.problemDate >= :fromDate ORDER BY dr.dailyProblem.problemDate DESC, dr.rankPosition ASC")
    List<DailyRanking> findRecentRankings(@Param("fromDate") LocalDate fromDate);
    
    // 특정 회원의 최근 N일간 랭킹 기록
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.member.id = :memberId AND dr.dailyProblem.problemDate >= :fromDate ORDER BY dr.dailyProblem.problemDate DESC")
    ...
    @Query("SELECT dr FROM DailyRanking dr WHERE dr.dailyProblem.id = :problemId AND dr.solveTime > :afterTime ORDER BY dr.solveTime ASC")
